...
func SetupAutoExport(ctx context.Context, opts ...AutoExportOption) (shutdown func(ctx context.Context) (err error), err error) {
	...
	shutdownFuncs = append(shutdownFuncs, func(ctx context.Context) error {
		if err := metricReader.Shutdown(ctx); err != nil &&
			// MEMO: meterProvider.Shutdown might have called metricReader.Shutdown internally
			!errors.Is(err, metric.ErrReaderShutdown) {
	...
	// log
	logExporter := c.logExporter
	if isNil(logExporter) {
		logExporter, err = autoexport.NewLogExporter(ctx, deepDistinct(c.logExporterOptions)...)
		if err != nil {
			return shutdown, fmt.Errorf("autoexport.NewLogExporter: %w", err)
		}
	}
	shutdownFuncs = append(shutdownFuncs, func(ctx context.Context) error {
		if err := logExporter.Shutdown(ctx); err != nil {
			return fmt.Errorf("logExporter.Shutdown: %w", err)
		}
		return nil
	})
	logProcessor := log.NewBatchProcessor(logExporter)
	shutdownFuncs = append(shutdownFuncs, func(ctx context.Context) error {
		if err := logProcessor.Shutdown(ctx); err != nil {
			return fmt.Errorf("logProcessor.Shutdown: %w", err)
		}
		return nil
	})
	loggerProvider := log.NewLoggerProvider(deepDistinct(append([]log.LoggerProviderOption{log.WithProcessor(logProcessor), log.WithResource(res)}, c.loggerProviderOptions...))...)
	shutdownFuncs = append(shutdownFuncs, func(ctx context.Context) error {
...
type autoexportConfig struct {
	...
	logExporterOptions    []autoexport.LogOption

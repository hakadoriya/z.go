...
func TestLFU_EvictionOrder(t *testing.T) {
	...
	// ja: key3: 2回
	cache.Get("key3")
	// key4: 1 time (Set only)
	// ja: key4: 1回（Set のみ）
	// key5: 1 time (Set only)
	// ja: key5: 1回（Set のみ）

	// Add new element (key4 or key5 should be evicted)
	// ja: 新しい要素を追加（key4 または key5 がエビクトされるはず）
	cache.Set("key6", "value6")

	// Either key4 or key5 should have been deleted
	// ja: key4 または key5 のいずれかが削除されているはず
	key4Exists := false
	key5Exists := false
	...
	// High frequency keys should still exist
	// ja: 頻度の高いキーは存在するはず
...
func TestLFU_UpdateIncreasesFrequency(t *testing.T) {
	...
	// Increase frequency for key1 and key2
	// ja: key1 と key2 の頻度を増加
	cache.Get("key1")
	cache.Get("key2")
	cache.Set("key1", "updated") // Set also increases frequency
	// ja: Set も頻度を増加させる

	// Add new element (key3 should be evicted)
	// ja: 新しい要素を追加（key3 がエビクトされるはず）

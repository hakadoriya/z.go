...
func Equal(tb testing.TB, printFunc func(args ...any), expected, actual interface{}, msgAndArgs ...interface{}) (success bool) {
	...
	if !reflect.DeepEqual(expected, actual) {
		...
		switch {
		case len(msgAndArgs) == 0:
			msg = fmt.Sprintf("❌: %s: expected != actual:\n--- EXPECTED\n+++ ACTUAL\n%s", tb.Name(), simplediffz.Diff(fmt.Sprintf("%+v", expected), fmt.Sprintf("%+v", actual)))
		case len(msgAndArgs) == 1:
			msg = fmt.Sprintf("❌: %s: %s:\n--- EXPECTED\n+++ ACTUAL\n%s", tb.Name(), fmt.Sprint(msgAndArgs...), simplediffz.Diff(fmt.Sprintf("%+v", expected), fmt.Sprintf("%+v", actual)))
		default:
			msg = fmt.Sprintf("❌: %s: %s:\n--- EXPECTED\n+++ ACTUAL\n%s", tb.Name(), fmt.Sprintf(fmt.Sprint(msgAndArgs[0]), msgAndArgs[1:]...), simplediffz.Diff(fmt.Sprintf("%+v", expected), fmt.Sprintf("%+v", actual)))
		}
...
func NotEqual(tb testing.TB, printFunc func(args ...any), unexpected, actual interface{}, msgAndArgs ...interface{}) (success bool) {
	...
	if reflect.DeepEqual(unexpected, actual) {
		...
		switch {
		case len(msgAndArgs) == 0:
			msg = fmt.Sprintf("❌: %s: expected == actual:\n--- UNEXPECTED\n+++ ACTUAL\n%s", tb.Name(), simplediffz.Diff(fmt.Sprintf("%+v", unexpected), fmt.Sprintf("%+v", actual)))
		case len(msgAndArgs) == 1:
			msg = fmt.Sprintf("❌: %s: %s:\n--- UNEXPECTED\n+++ ACTUAL\n%s", tb.Name(), fmt.Sprint(msgAndArgs...), simplediffz.Diff(fmt.Sprintf("%+v", unexpected), fmt.Sprintf("%+v", actual)))
		default:
			msg = fmt.Sprintf("❌: %s: %s:\n--- UNEXPECTED\n+++ ACTUAL\n%s", tb.Name(), fmt.Sprintf(fmt.Sprint(msgAndArgs[0]), msgAndArgs[1:]...), simplediffz.Diff(fmt.Sprintf("%+v", unexpected), fmt.Sprintf("%+v", actual)))
		}

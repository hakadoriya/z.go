...
func TestRetryer_Retry(t *testing.T) {
	...
	t.Run("success,constant", func(t *testing.T) {
		...
		const maxRetries = 20
		ctx := context.Background()
		r := New(ctx, NewConfig(1*time.Microsecond, 999*time.Microsecond, WithMaxRetries(maxRetries), WithJitter(DefaultJitter(WithDefaultJitterRange(1*time.Millisecond, 10*time.Millisecond), WithDefaultJitterRand(rand.New(rand.NewSource(0))))))) //nolint:gosec
		buf := bytes.NewBuffer(nil)
		for r.Retry() {

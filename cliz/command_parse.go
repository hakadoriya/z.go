	...
	for i := 0; i < len(osArgs); i++ {
		...
		case strings.HasPrefix(osArg, shortOptionPrefix):
			for _, opt := range c.Options {
				...
				case *StringOption:
					...
					case argIsHyphenOption(o, osArg):
						if hasNoOptionValue(osArgs, i) {
							return nil, errorz.Errorf("%s: %w", osArg, clierrz.ErrMissingOptionValue)
						}
				...
				default:
					return nil, errorz.Errorf("%s: %w", osArg, clierrz.ErrInvalidOptionType)
				}
			}
			return nil, errorz.Errorf("%s: %w", osArg, clierrz.ErrUnknownOption)
		default:
			if subcmd := c.getSubcommand(osArg); subcmd != nil {
				subcmd.allExecutedCommandNames = c.allExecutedCommandNames
				defer func() { c.allExecutedCommandNames = subcmd.allExecutedCommandNames }()

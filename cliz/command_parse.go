	...
	for i := 0; i < len(osArgs); i++ {
		...
		case strings.HasPrefix(osArg, shortOptionPrefix):
			for _, opt := range c.Options {
				...
				case *StringOption:
					...
					case argIsHyphenOption(o, osArg):
						if hasNoOptionValue(osArgs, i) {
							return nil, errorz.Errorf("%s: %w", osArg, clicorez.ErrMissingOptionValue)
						}
				...
				default:
					return nil, errorz.Errorf("%s: %w", osArg, clicorez.ErrInvalidOptionType)
				}
			}
			return nil, errorz.Errorf("%s: %w", osArg, clicorez.ErrUnknownOption)
		default:

...
func TestFromContext(t *testing.T) {
	...
	t.Run("error,ErrNilContext", func(t *testing.T) {
		...
		if _, err := FromContext(nil); !errors.Is(err, clierrz.ErrNilContext) {
	...
	t.Run("error,ErrCommandNotSetInContext", func(t *testing.T) {
		...
		if _, err := FromContext(context.Background()); !errors.Is(err, clierrz.ErrNotSetInContext) {
		...
		defer func() {
			if r := recover(); r == nil {
				...
				if err, ok := r.(error); ok {
					...
					if !errors.Is(err, clierrz.ErrNilContext) {
						t.Errorf("❌: err != ErrNilContext: %+v", err)
					}
				}
			}

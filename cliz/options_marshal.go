...
func MarshalOptions(v interface{}, opts ...MarshalOptionsOption) (options []Option, err error) {
	...
	for i := range ref.NumField() {
		...
		if !fieldValue.CanSet() {
			return nil, fmt.Errorf("type=%s: field=%s: tag=%s: %w", typ, field.Name, cfg.tagKey, clicorez.ErrStructFieldCannotBeSet)
		}

		tagValue := stringz.TrimLeftSpace(field.Tag.Get(cfg.tagKey))
		clicorez.Logger.Debug(fmt.Sprintf("type=%s: field=%s: tag=%s, tagValue=%s", typ, field.Name, cfg.tagKey, tagValue))
		if tagValue == "" {
			continue
		}

		optName, opts := parseTagValue(tagValue)
		clicorez.Logger.Debug(fmt.Sprintf("type=%s: field=%s: tag=%s, optName=%s, opts=%v", typ, field.Name, cfg.tagKey, optName, opts))
		if optName == "" {
			return nil, fmt.Errorf("type=%s: field=%s: tag=%s: tagValue=%s: %w", typ, field.Name, cfg.tagKey, tagValue, clicorez.ErrInvalidTagValue)
		}
		...
		default:
			return nil, fmt.Errorf("type=%s: field=%s: fieldType=%s: tag=%s: %w", typ, field.Name, field.Type, cfg.tagKey, clicorez.ErrStructFieldTypeNotSupported)
		}
	}

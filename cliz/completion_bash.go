...
func (c *Command) initAppendGenerateBashCompletionSubCommands() {
	...
	c.SubCommands = append(
		...
		//nolint:exhaustruct
		&Command{
			Name:   clicorez.DefaultGenerateBashCompletionSubCommandName,
			Hidden: true,
			ExecFunc: func(_ context.Context, _ Cmd, _ []string) error {
				...
				_, _ = io.WriteString(c.GetStdout(), strings.Join(completions, " ")+"\n")
				return nil
			},
		})
}

	...
	for _, opt := range c.Options {
		...
		if name := opt.GetName(); name != "" {
			if key, alreadyExists := duplicateChecker[name]; alreadyExists {
				return errorz.Errorf("option: %s(%s) and %s(%s%s): %w", key.cmdName, key.optionFullName, c.Name, longOptionPrefix, name, clicorez.ErrDuplicateOption)
			}
		...
		for _, alias := range opt.GetAliases() {
			if alias != "" {
				if key, alreadyExists := duplicateChecker[alias]; alreadyExists {
					return errorz.Errorf("option: %s(%s) and %s(%s%s): %w", key.cmdName, key.optionFullName, c.Name, shortOptionPrefix, alias, clicorez.ErrDuplicateOption)
				}
		...
		if env := opt.GetEnv(); env != "" {
			...
			if key, alreadyExists := duplicateChecker[env]; alreadyExists {
				return errorz.Errorf("option: %s(%s) and %s(%s%s): %w", key.cmdName, key.optionFullName, c.Name, envPrefix, env, clicorez.ErrDuplicateOption)
			}

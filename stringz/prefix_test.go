...
`
		...
		if expected, actual := after, AddPrefix("prefix ", before, "\n"); expected != actual {
	...
	t.Run("success,EmptyPrefix", func(t *testing.T) {
		...
		const before = `a`
		const after = `a`
		if expected, actual := after, AddPrefix("", before, "\n"); expected != actual {
			t.Errorf("❌: expected != actual:\n---EXPECTED\n%s\n+++ACTUAL\n%s", expected, actual)
		}
	})
...
c`
		...
		if expected, actual := after, AddPrefix("prefix ", before, ""); expected != actual {
			t.Errorf("❌: expected != actual:\n---EXPECTED\n%s\n+++ACTUAL\n%s", expected, actual)
		}
	})
}
